...
-- Counts all references to given object
...
-- Prints all references to given object
...
function traverse(funcs, ignoreobjs)
  ...
  while not List.isempty(env.list) do
    ...
    if not gc["traverse" .. t] then
      if not unknown_types[t] then
        utils.warn("[ltraverse] No traverse method for type: " .. t)
        unknown_types[t] = true
      end
    else
